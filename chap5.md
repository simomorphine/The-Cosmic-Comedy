...
\[
\Gamma \propto \exp\left( -\frac{2L}{\hbar} \sqrt{2m(\Delta U - E)} \right)
\]

**In ML terms:**

\[
\boxed{\Gamma \propto \exp\left( -\frac{2L}{\eta} \sqrt{2 \lambda (\Delta U - \mathcal{L})} \right)}
\]
...
\[
\boxed{t_{\text{escape}} \propto \exp\left( \frac{2L}{\eta} \sqrt{2\lambda \Delta U} \right)}
\]
...
For a **quadratic loss** \( U(w) = \frac{1}{2} \lambda w^2 \), the quantum solution is:

**Energy levels:**
\[
E_n = \hbar \omega \left( n + \frac{1}{2} \right), \quad \omega = \sqrt{\lambda/m}
\]
...
**Algorithm:**
\[
w_{k+1} = w_k - \eta \nabla U(w_k) + \sigma \cdot \text{Lévy}(\alpha)
\]
...
**Algorithm:**
\[
\nabla_{\text{quantum}} U = \frac{\int \nabla U(w) |\psi(w)|^2 dw}{\int |\psi(w)|^2 dw}
\]
...
\[
\boxed{\eta \approx \frac{\hbar^2}{m \lambda}}
\]

| Regime | Behavior | ML Equivalent |
|--------|----------|---------------|
| **Classical (\( \eta \gg \hbar^2/(m\lambda) \))** | Particle obeys Newton's laws | SGD with large LR |
| **Quantum (\( \eta \approx \hbar^2/(m\lambda) \))** | Particle tunnels through barriers | Adam with moderate LR |
| **Too quantum (\( \eta \ll \hbar^2/(m\lambda) \))** | Particle spreads everywhere | SGD with tiny LR (random) |
...
> \[
> \Gamma \propto \exp\left( -\frac{2L}{\eta} \sqrt{2\lambda(\Delta U - \mathcal{L})} \right)
> \]
...
> 
> \[
> \psi_0(w) = \left( \frac{m\omega}{\pi\hbar} \right)^{1/4} e^{-m\omega w^2/(2\hbar)}
> \]
> 
...
> \]
> 
> **Hint:** Use the uncertainty principle \( \Delta w \Delta p \geq \hbar/2 \).
...
\[
\boxed{\psi(w, t) = \sum_n c_n \psi_n(w) e^{-iE_n t/\hbar}}
\]

**The wavefunction of a weight particle:**

\[
\boxed{|\psi(w, t)|^2 = \text{Probability of finding the weight at } w \text{ at time } t}
\]

**The Schrödinger equation for learning:**

\[
\boxed{i\hbar \frac{\partial \psi}{\partial t} = \left( -\frac{\hbar^2}{2m} \frac{\partial^2}{\partial w^2} + U(w) \right) \psi}
\]
